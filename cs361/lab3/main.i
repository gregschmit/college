...
int main( int argc, char ** argv ) {
    ...
    for ( i = 0; i < N; i++ ) {
        ...
        if ( ((((__extension__ (((union { __typeof(child_status) __in; int __i; }) { .__in = (child_status) }).__i))) & 0x7f) == 0) )
            printf( "Child %d terminated with exit status %d\n",
                   wpid, ((((__extension__ (((union { __typeof(child_status) __in; int __i; }) { .__in = (child_status) }).__i))) & 0xff00) >> 8) );
        else
