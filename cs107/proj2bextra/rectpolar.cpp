 ...
 *
 *              INCLUDES EXTRA CREDIT AT BOTTOM
 */
...
int max_min(double arr[], int len, bool max)
/*
 * PRE: input an array of doubles, length of the array > 0, and max (1 for 
 *      maximum, 0 for minimum)
 * PST: output max or min value index (first val foound) - not neccessarily 
 *      unique
 */
{
        int index = 0;
 ...
 */
{
        ...
        x0 = x[max_min(x, cur_data, 0)];
        x1 = x[max_min(x, cur_data, 1)];
        y0 = y[max_min(y, cur_data, 0)];
        y1 = y[max_min(y, cur_data, 1)];

        r0 = r[max_min(r, cur_data, 0)];
        r1 = r[max_min(r, cur_data, 1)];
        theta0 = theta[max_min(theta, cur_data, 0)];
        theta1 = theta[max_min(theta, cur_data, 1)];

        if (SHOW_USER_CONTENT) {
                cout << endl << "The smallest rectangle that encloses all of"
                        " the inputs (assuming you mean validated inputs in"
                        " Quadrant I, before the shift) is:" << endl << "from"
                        " x=" << x0 << " to x=" << x1 << endl << "and" << endl
                        << "from y=" << y0 << " to y=" << y1 << endl;
                cout << endl << "The smallest circular segment that encloses"
                        " all of the data after the shift has radius r=" <<
                        r1 << " and angles:" << endl << "from theta=" <<
                        theta0*180.0/M_PI << " to theta=" << theta1*180.0/M_PI
                        << endl;
        }
